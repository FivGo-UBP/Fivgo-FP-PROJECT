...
export class ChatbotBantuanPage implements OnInit, AfterViewChecked {
  ...
  chatTree: ChatNode[] = [
    {
      ...
      children: [
        {
          label: 'Perilaku penumpang saat berkendara',
          children: [
            { label: 'Penumpang mengganggu konsentrasi berkendara', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
            { label: 'Penumpang bertindak membahayakan', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
            { label: 'Penumpang tidak menggunakan helm/seatbelt', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
            { label: 'Penumpang meminta mengebut/melanggar rambu', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
        ...
        },
        {
          ...
          children: [
            ...
            { label: 'Kerusakan pada kendaraan', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
            { label: 'Lainnya', children: [{ label: 'Lapor via Formulir' }, { label: 'Akhiri Chat' }] }
          ]
        },
        {
          ...
          children: [
            { label: 'Pelecehan seksual oleh penumpang', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
    ...
    },
    {
      ...
      label: 'Lapor perilaku Penumpang',
      children: [
        {
          label: 'Sikap Penumpang',
          children: [
            { label: 'Penumpang bersikap kasar / tidak sopan', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
            { label: 'Penumpang mengotori / merusak bagian dalam kendaraan', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
            { label: 'Penumpang membawa hewan peliharaan tanpa izin', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
            { label: 'Lainnya', children: [{ label: 'Lapor via Formulir' }, { label: 'Akhiri Chat' }] }
          ]
        },
        {
          label: 'Ketidaksesuaian Data / Kapasitas',
          children: [
            { label: 'Jumlah penumpang melebihi kapasitas kendaraan', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
            { label: 'Muatan barang terlalu banyak / berat', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
            { label: 'Penumpang berbeda dengan akun pemesan', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
            { label: 'Lainnya', children: [{ label: 'Lapor via Formulir' }, { label: 'Akhiri Chat' }] }
          ]
        },
        {
          ...
          children: [
            { label: 'Penumpang menolak membayar tunai', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
            { label: 'Penumpang kurang membayar tarif', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
            { label: 'Penumpang meminta rute tambahan tanpa aplikasi', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
            { label: 'Lainnya', children: [{ label: 'Lapor via Formulir' }, { label: 'Akhiri Chat' }] }
          ]
        }
      ]
      ...
      children: [
        {
          ...
          children: [
            ...
            { label: 'Gagal bayar non-tunai oleh penumpang', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
            { label: 'Penumpang kurang / lupa membayar', children: [{ label: 'Kirim Laporan' }, { label: 'Akhiri Chat' }] },
            { label: 'Lainnya', children: [{ label: 'Lapor via Formulir' }, { label: 'Akhiri Chat' }] }
          ]
        },
        {
  ...
  ngOnInit() {
    ...
    this.authService.currentUser.subscribe(user => {
      this.user = user;
      this.userEmail = user?.email || 'driver@fivgo.com';
    });
  ...
  private loadOrderAndStart() {
    ...
    this.orderService.getHistoryDetail(this.orderId!).subscribe({
      next: (res) => {
        ...
        this.selectedPath = [];
        this.messages = [];
        const namaUser = this.user?.name || 'Driver';

        // Tampilkan bubble pertama: Halo Rekan [Nama] 👋
        this.pushBotMessage(`Halo Rekan ${namaUser} 👋`);
...
${orderDetailsStr}
      ...
      this.formService.submitLaporanMasalah({
        ...
        next: () => {
          ...
          if (mainCategoryKey === 'keamanan') {
            ...
            responseMsg = `Laporan barang tertinggal (${detailKejadian}) berhasil dibuat. Kami akan segera berkoordinasi dengan Penumpang untuk mengembalikan barang tersebut.`;
          } else if (mainCategoryKey === 'driver_kendaraan') {
            responseMsg = `Laporan perilaku Penumpang (${detailKejadian}) berhasil terkirim. Kami sangat menghargai laporan Anda untuk menjaga kenyamanan berkendara.`;
          } else {
  ...
  submitBantuanForm() {
    ...
    if (this.activeFormType === 'keamanan') {
      ...
      categoryTitle = 'Lapor perilaku Penumpang';
      ...
      selectedDetails = `Alasan Batal/Pembayaran: ${this.formBatalJalan.cancelReason}`;
