...
export class FormTelponLoginPage implements OnInit {
  ...
  async onSubmit() {
    ...
    this.authService.requestOtp(fullPhone, 'driver').subscribe({
      next: (res: any) => {
        loading.dismiss();
        localStorage.setItem(`otp_expires_at_driver_${fullPhone}`, String(Date.now() + (res.expired_in_seconds || 300) * 1000));
        // Pass phone to OTP page
        this.router.navigate(['/kode-otp-login'], {
          queryParams: { phone: fullPhone, role: 'driver', purpose: 'login' }
        });
      },
      error: (err: any) => {
        loading.dismiss();
        if (err.status === 404) {
          this.showAlert('Nomor Belum Terdaftar', 'Nomor belum terdaftar. Pendaftaran akun driver hanya dapat dilakukan oleh admin.');
          return;
        }
